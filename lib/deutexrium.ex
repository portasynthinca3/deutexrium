defmodule Deutexrium do
  ...
  def handle_event({:INTERACTION_CREATE, %Struct.Interaction{data: %{name: name}} = inter, _}) when name == "settings" or name == "first_time_setup" do
    {content, components} = Server.Settings.initialize(inter)
    Api.create_interaction_response!(inter, %{type: 4, data: %{content: content, components: components, flags: 64}})
  end
  ...
  def handle_event({:INTERACTION_CREATE, %Struct.Interaction{data: %{component_type: t, custom_id: id}} = inter, _}) when t == 2 or t == 8 do
